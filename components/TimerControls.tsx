...
const ControlButton: React.FC<{ onClick: () => void; children: React.ReactNode; ariaLabel: string, className?: string, disabled?: boolean }> = 
  ...
  <button
    ...
    disabled={disabled}
    className={`p-3 sm:p-4 rounded-full glass-bg border border-white/20 hover:bg-white/30 focus:outline-none focus:ring-2 focus:ring-white/50 transition-all duration-200 ease-in-out transform hover:scale-105 active:scale-95 ${disabled ? 'opacity-50 cursor-not-allowed hover:scale-100 active:scale-100' : ''} ${className || ''}`}
  >
    {children}
  </button>
);
  ...
  return (
    <div className="flex justify-center items-center space-x-4 sm:space-x-6 my-4 sm:my-6 md:my-8">
      <ControlButton 
        ...
        className={(timerState === TimerState.IDLE && timeLeft === initialDuration) ? 'opacity-60' : ''} // Keep visual cue if needed, though disabled handles it
      >
        <ResetIcon className="w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 text-white" />
      </ControlButton>
      <ControlButton 
        onClick={onStartPause} 
        ariaLabel={timerState === TimerState.RUNNING ? "Pause Timer" : "Start Timer"}
        className="w-16 h-16 sm:w-20 sm:h-20 !rounded-full !p-0 flex items-center justify-center bg-white/30 hover:bg-white/40 shadow-lg"
      >
